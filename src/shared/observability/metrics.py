...
class MetricsStorage:
    ...
    _storage: DefaultDict[str, _MetricObject] = collections.defaultdict(_MetricObject)
    ...
    def get_full_key(self, key: str = "", label: str = ""):
        if not self.metric_prefix:
            return key
        return f"{self.metric_prefix}.{key}{label}"

    def increment(self, key: str = "", delta: float = 1.0, label: str = ""):
        with self._lock:
            metric = self._storage[self.get_full_key(key, label=label)]
            metric.value += delta
            metric.label = label

    def avg(self, key: str = "", value: float = 1.0, count: int = 1, label: str = ""):
        with self._lock:
            metric = self._storage[self.get_full_key(key, label=label)]
            metric.value += value
            metric.count += count
    ...
    def set(self, key: str = "", value: float = 1.0):
        with self._lock:
