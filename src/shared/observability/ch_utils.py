...
http_pool: httputil.PoolManager | None = None
...
class Inserter(BaseWorker, metaclass=Singleton):
    INTERVAL = 15

    metrics: "observability.metrics.MetricsStorage" = None  # round imports
    ...
    def insert(self, record: "observability.schemas.BaseStructure") -> None:
        self.queue.put_nowait(record)

    async def insert_wait(self, record: "observability.schemas.BaseStructure") -> None:
        await self.queue.put(record)

    async def insert_sync(
        self,
        *records: "observability.schemas.BaseStructure"
    ) -> None:
        try:
    ...
    async def flush(self) -> None:
        with Timer() as t:
            async with self.lock:
                batch: list["observability.schemas.BaseStructure"] = []
            ...
            with tracer.start_as_current_span(
                "flush",
                attributes=dict(table_name=self.table_name)
            ):
                try:
...
class HTTPDictCursor:
    ...
    async def _execute(
        ...
        result = await self.client.query(
            ...
            args,
            query_tz=pytz.UTC,
            settings=dict(session_id=self.session_id)
        )

        if "total_rows_to_read" in result.column_names:  # todo: this is kostil! thanks, clickhouse-connect
            return
...
T = tp.TypeVar("T", bound="common.structures.BaseStructure")
